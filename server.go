		...
		if !p.Inbound() {
			...
			if !cfg.DisableListen /* && isCurrent? */ {
				...
				if addrmgr.IsRoutable(lna) {
					...
					pInfo.knownAddrMutex.Lock()
					if _, exists := pInfo.knownAddresses[addrmgr.NetAddressKey(lna)]; !exists {
						addresses := []*wire.NetAddress{lna}
						p.PushAddrMsg(addresses)
						pInfo.knownAddresses[addrmgr.NetAddressKey(lna)] = struct{}{}
					}
					pInfo.knownAddrMutex.Unlock()
				}
			}
...
func (s *server) handleTxMsg(p *peer.Peer, msg *wire.MsgTx) {
	...
	s.blockManager.QueueTx(tx, pInfo)
	<-pInfo.txProcessed
}
...
func (s *server) handleBlockMsg(p *peer.Peer, msg *wire.MsgBlock, buf []byte) {
	...
	s.blockManager.QueueBlock(block, pInfo)
...
func (s *server) handleInvMsg(p *peer.Peer, msg *wire.MsgInv) {
	...
	s.blockManager.QueueInv(msg, pInfo)
}
...
func (s *server) handleHeadersMsg(p *peer.Peer, msg *wire.MsgHeaders) {
	...
	s.blockManager.QueueHeaders(msg, pInfo)
}
...
func (s *server) handleGetAddrMsg(p *peer.Peer, msg *wire.MsgGetAddr) {
	...
	for _, na := range addrCache {
		pInfo.knownAddrMutex.Lock()
		if _, exists := pInfo.knownAddresses[addrmgr.NetAddressKey(na)]; !exists {
	...
	for _, na := range addresses {
		pInfo.knownAddrMutex.Lock()
		pInfo.knownAddresses[addrmgr.NetAddressKey(na)] = struct{}{}
		pInfo.knownAddrMutex.Unlock()
	}
}
...
func (s *server) handleRelayInvMsg(state *peerState, msg relayMsg) {
	state.forAllPeers(func(p *peer.Peer) {
		...
		if msg.invVect.Type == wire.InvTypeTx {
			...
			if s.blockManager.RelayTxDisabled(pInfo) {
				return
			}
...
func (s *server) handleQuery(querymsg interface{}, state *peerState) {
	...
	case connectNodeMsg:
		...
		if s.handleAddPeerMsg(state, p, msg.permanent) {
			msg.reply <- nil
		} else {
...
func (s *server) addPeer(addr string) *peer.Peer {
	...
	// Setup a temporary address that we are connecting to with faked up
	// service flags.  We will replace this with the real one after version
	// negotiation is successful.  The only failure case here would be if the
	// string was incomplete for connection so can't be split into address and
	// port, and thus this would be invalid anyway.  In which case we return
	// nil to be handled by the caller.  This must be done before we fork off
	// the goroutine because as soon as this function returns the peer must
	// have a valid netaddress.
	...
	na, err := s.addrManager.HostToNetAddress(host, uint16(port), 0)
	if err != nil {
